...
export const ProjectsSection: React.FC<ProjectsSectionProps> = ({ onOpenVideo, onOpenContact }) => {
      ...
      <div className="w-full max-w-[1440px] mx-auto flex flex-col gap-12 lg:gap-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 items-start">
            {shortformReels.map((reel, idx) => (
              <div key={idx} className="flex flex-col sm:flex-row gap-4 items-stretch group">
                {/* 9:16 Video Frame Card with YouTube / Video Preview */}
                <div 
                  onClick={() => onOpenVideo(`https://www.youtube-nocookie.com/embed/${reel.videoId}?autoplay=1`)}
                  className="bg-[#474747] hover:bg-[#3d3d3d] w-full sm:w-[215px] h-[383px] rounded-[12px] p-3 flex flex-col justify-between cursor-pointer relative overflow-hidden shrink-0 shadow-lg transition-all duration-300 border border-transparent hover:border-[#39adca]"
                >
        ...
        <div className="flex flex-col gap-6 pt-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
            {/* 16:9 Video Card with YouTube Preview */}
            <div 
              onClick={() => onOpenVideo(`https://www.youtube-nocookie.com/embed/${longformProjects[longformIndex % longformProjects.length].videoId}?autoplay=1`)}
              className="lg:col-span-6 bg-[#474747] hover:bg-[#3d3d3d] aspect-video min-h-[300px] lg:min-h-[388px] rounded-[12px] p-4 flex flex-col justify-between cursor-pointer relative overflow-hidden shadow-xl group transition-all border border-transparent hover:border-[#39adca]"
            >
